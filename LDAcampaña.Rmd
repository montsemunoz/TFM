...
lda_11g <- LDA(dtm, k = 11, method = 'Gibbs', control = list(seed = list(1505)))
lda_12g <- LDA(dtm, k = 12, method = 'Gibbs', control = list(seed = list(1505)))
lda_14g <- LDA(dtm, k = 14, method = 'Gibbs', control = list(seed = list(1505)))
```
...
lda_td11g <- tidy(lda_11g, matrix = "beta")
lda_td12g <- tidy(lda_12g, matrix = "beta")
lda_td14g <- tidy(lda_14g, matrix = "beta")
```
...
terminos_frecuentes11g <- lda_td11g %>% group_by(topic) %>% ungroup() %>% arrange(topic, -beta)
top11g <- terminos_frecuentes11g %>% group_by(topic) %>% top_n(10, beta) 
...
terminos_frecuentes14g <- lda_td14g %>% group_by(topic) %>% ungroup() %>% arrange(topic, -beta)
top14g <- terminos_frecuentes14g %>% group_by(topic) %>% top_n(10, beta)
```
...
```{r}
ap_top_terms11g <- lda_td11g %>% group_by(topic) %>% slice_max(beta, n = 10) %>% ungroup() %>% arrange(topic, -beta)

ap_top_terms11g %>% mutate(term = reorder_within(term, beta, topic)) %>% ggplot(aes(beta, term, fill = factor(topic))) +
geom_col(show.legend = FALSE) + facet_wrap(~ topic, scales = "free") + scale_y_reordered()
```
...
```{r}
ap_top_terms14g <- lda_td14g %>% group_by(topic) %>% slice_max(beta, n = 10) %>% ungroup() %>% arrange(topic, -beta)

ap_top_terms14g %>% mutate(term = reorder_within(term, beta, topic)) %>% ggplot(aes(beta, term, fill = factor(topic))) +
geom_col(show.legend = FALSE) + facet_wrap(~ topic, scales = "free", ncol=4) + scale_y_reordered()
```
...
lda_gamma14g <- tidy(lda_14g, matrix = "gamma")
```
...
lda_14i <- LDA(dtm, k = 14, method = 'VEM', control = list(seed = list(1505)))
```
...
lda_td14i <- tidy(lda_14i, matrix = "beta")
```
...
terminos_frecuentes14i <- lda_td14i %>% group_by(topic) %>% top_n(10, beta) %>% ungroup() %>% arrange(topic, -beta)
```
...
```{r}
ap_top_terms12i <- lda_td12i %>% group_by(topic) %>% slice_max(beta, n = 4) %>% ungroup() %>% arrange(topic, -beta)
...
```{r}
ap_top_terms14i <- lda_td14i %>% group_by(topic) %>% slice_max(beta, n = 10) %>% ungroup() %>% arrange(topic, -beta)

ap_top_terms14i %>% mutate(term = reorder_within(term, beta, topic)) %>% ggplot(aes(beta, term, fill = factor(topic))) +
geom_col(show.legend = FALSE) + facet_wrap(~ topic, scales = "free", ncol=4) + scale_y_reordered()
```
